...
struct at_cmd_t at_cmds[] = {
	...
	{AT_NOISE_CANCEL,            "AT_NOISE_CANCEL",            "AT+KNOISE=0,0"},
	{AT_SPEAKER,                 "AT_SPEAKER",                 "AT+SPEAKER=9"},
	{AT_DTMF_DETECTION,	     "AT_DTMF_DETECTION",          "AT+WDDM=1"},
	...
	{AT_GET_WAITING,             "AT_GET_WAITING",             "+CCWA: \"$WAITING_NUMBER\""},
	...
	{AT_CMIC,                    "AT_CMIC",                    "AT+QMIC=0,10"},	/*Makes Add 2012-4-5 17:44*/
	{AT_CLVL,                    "AT_CLVL",                    "AT+CLVL=80"},	/*Makes Add 2012-4-5 17:44*/
	{AT_ECHOCANSUP,              "AT_ECHOCANSUP",              "AT+KECHO=0"},	/*Makes Add 2012-4-5 17:44*/
	{AT_CALL_NOTIFICATION,	     "AT_CALL_NOTIFICATION",       "AT*PSCSCN=1"},	
	{AT_MODE,                    "AT_MODE",                    "ATX4"},	/*Makes Add 2012-4-10 15:50*/
	{AT_CMUX,                    "AT_CMUX",                    "AT+CMUX=0"},	/*Makes Add 2012-4-10 15:50*/
	{AT_GENERAL_INDICATION,	     "AT_GENERAL_INDICATION",      "AT+WIND=255"},	
	{AT_RESET,		     "AT_RESET",      		   "AT+CFUN=1"},	
	{AT_POWER_OFF,		     "AT_POWER_OFF",  		   "AT+CFUN=0"},	
	{AT_UPDATE_1,		     "AT_UPDATE_1",                "NO UPDATE"},	
	{AT_UPDATE_2,		     "AT_UPDATE_2",                "NO UPDATE"},	
	{AT_UPDATE_CMD,		     "AT_UPDATE_CMD",              "AT"},	
	{AT_UPDATE_SPAN,	     "AT_UPDATE_SPAN",             "0"},
/*	{AT_SIM_TOOLKIT,	     "AT_SIM_TOOLKIT",             "AT+STSF=0"},	*/
	{AT_SIM_TOOLKIT,	     "AT_SIM_TOOLKIT",             "AT"},
	{AT_CREG_DISABLE,	     "AT_CREG_DISABLE",            "AT+CREG=0"},
	{AT_SIM_SELECT_1,	     "AT_SIM_SELECT_1",            "AT+WIOM=1,\"GPIO22\",1,0"},
	{AT_SIM_SELECT_2,	     "AT_SIM_SELECT_2",            "AT+WIOM=1,\"GPIO23\",1,0"},
	{AT_SIM_SELECT_3,	     "AT_SIM_SELECT_3",            "AT+WIOM=4"},
	{AT_SET_GAIN_INDEX,	     "AT_SET_GAIN_INDEX",          "AT+WBHV=8,0"},
	{AT_DEL_MSG,                 "AT_DEL_MSG",                 "AT+CMGD=1,4"},	
	{AT_SEND_USSD,               "AT_SEND_USSD",               "AT+CUSD=1,\"$USSD_CODE\""},
	{AT_CHECK_USSD,              "AT_CHECK_USSD",              "+CUSD:"},	
	{AT_CHECK_BAUD,              "AT_CHECK_BAUD",              "AT+IPR/"},	
...
static char* str_check_symbol(const char* src, char* buf, int len)
{
	if(src == NULL ||
       buf == NULL ||
       len <= 0 ) {
...
{
	module_id=0; // FIXME: rightnow static for Sierra module.
...
#ifdef WAVECOM
/******************************************************************************
 * String Comparation From a Given List
 *      List Present in /etc/allo/allog4c/response/
 *      This function is intended for comparing response received form GSM CARD.
 *      If response is coroupted, chances are it wont be matching with
 *      expected response, so it will enable us to refire the AT command to
 *      keep State machine running.
 *      (pawan allo.com)
 * param:
 *              buf: source string, response received.
 * return:
 *              0: equal
 *              1: not equal 
 * e.g.         
 ...
 *              WDDI
 *
 *              expectlist_compare("WDDI")              => 0
 *              expectlist_compare("+WDDI: "*",60")     => 0
 *              expectlist_compare("+WAAI: "*",60")     => 1
 *              expectlist_compare("abc")               => 1
 ******************************************************************************/
int expectlist_compare (char *buf){
        int i=0, ret=1;
        for (i=0;i<MAX_EXPECTLIST_SIZE;++i){
 //               printf("i=%d val= %s \n", i, list[i].value);
                ret = NULL==strstr(buf, list[i].value) ? 1 : 0;
                if(!ret)
                        return ret;
        }
       // printf("+++++++++++expectlist buf= %s \n",buf);
        return 1;
}
#endif
...
char* get_echocansup_str(int module_id, char *cmd, int gain, char* buf, int len){
	char temp[1024];
	
	buf[0] = '\0';
		
	if(!str_check_symbol(cmd,temp,sizeof(temp)))
		return NULL;
	
	if(!str_replace(temp,temp,sizeof(temp),"$ECHOCANVAL","%d"))
		return NULL;
	
	snprintf(buf,len,temp,gain);
	
	return buf;
}

char* get_gain_str(int module_id, char *cmd, int gain, char* buf, int len)
{
	char temp[1024];
	...
	if(!str_replace(temp,temp,sizeof(temp),"$GAIN","%d"))
		return NULL;
...
#ifdef SUJ
	printf("Destination number is %s\n",buf);	
#endif
	return buf;
}
...
char * get_waiting(int module_id, char* h, char* num, int numLen, char * name, int nameLen)
{
	char temp[1024];
	
	if(!str_check_symbol(get_at(module_id, AT_GET_WAITING),temp,sizeof(temp)))
		return NULL;
	
	if(!str_replace(temp,temp,sizeof(temp),"$WAITING_NUMBER","%64s"))
		return NULL;

	char *endflag = NULL;
	if(sscanf(h, temp, num)) {
		endflag = strchr(num, '"');
		if (endflag)
			*endflag = '\0';
	}
...
{
#define CFGS_DIR "/etc/allo/allog4c/modules"
	struct dirent* dirp;
	DIR *dp;
	struct stat statbuf;
	char file_name[256];
...
#ifdef WAVECOM
#define EXPECTLIST_FILE "/etc/allo/allog4c/response/expectlist.conf"
        FILE *file = fopen(EXPECTLIST_FILE , "r"); /* read from the file in read mode */
        if ( file != NULL ){
                char line[25];
                int j=0;
                while ( fgets(line, 25, file) != NULL ) /* read all records */
                {
                        line[strlen(line) - 1]='\0';
                        strcpy(list[j].value, line);
                        ++j;
                }
                MAX_EXPECTLIST_SIZE=j;
                fclose(file);
                return 0;
        }else{
                printf("Can not Open expectinglist.conf\n");
        }
