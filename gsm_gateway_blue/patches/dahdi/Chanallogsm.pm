package Dahdi::Config::Gen::Chanallogsm;
use strict;
...
sub new($$$) {
	...
	my $file = $ENV{CHAN_AGSM_CHANNELS_FILE} || "/etc/asterisk/allogsm-channels.conf";
	my $self = {
			FILE	=> $file,
			GCONFIG	=> $gconfig,
			GENOPTS	=> $genopts,
...
sub generate($) {
	...
	print <<"HEAD";
; Dahdi Channels Configurations (chan_allogsm.conf)
;
; This is not intended to be a complete chan_allogsm.conf. Rather, it is intended
; to be #include-d by /etc/chan_allogsm.conf that will include the global settings
;
...
chanallogsm - Generate configuration for chan_allogsm channels.

=head1 SYNOPSIS

 use Dahdi::Config::Gen::Chanallogsm;

 my $cfg = new Dahdi::Config::Gen::Chanallogsm(\%global_config, \%genopts);
 $cfg->generate(@span_list);

=head1 DESCRIPTION

Generate the F</etc/asterisk/allogsm-channels.conf>
This is used as a configuration for asterisk(1).
It should be included in the main F</etc/asterisk/chan_allogsm.conf>.

Its location may be overriden via the environment variable 
C<CHAN_AGSM_CHANNELS_FILE>.
