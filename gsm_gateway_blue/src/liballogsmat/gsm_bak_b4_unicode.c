...
int gsm_pdu2sm_event(struct allogsm_modul *gsm, char *pdu) 
{
	...
	if (pdu_info.tp_dcs == 0x08) {
		...
		if(pdu_info.tp_ud[1] == '5') //for UDH with lenth 5
...
static int gsm_encode_pdu(const char* SCA, const char* TPA, char TP_PID, char TP_DCS, const char* TP_UD, char* pDst){
	...
	buf[2] = 0;
	if(TP_DCS == GSM_7BIT) {
                nLength = gsmEncode7bit(TP_UD, &buf[10], nLength+1, &buf[3]);
/*********************** UDH*************/	
		buf[3] = (char)(buf[3]+7);
                buf[4] = 0x05;
                buf[5] = 0x00;
...
#else
#ifdef PDU_LONG
int allogsm_encode_pdu_ucs2(const char* smsc, const char* dest, char* sms_data, const char* coding, gsm_sms_pdu* long_pdu, char* pdu){
#else
