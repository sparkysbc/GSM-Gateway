# !/bin/bash
#
# install.sh - Allo GSM Installation Script for AlloG4C 
#
# $Id$
#
...
banner()
{
	...
	echo -e "########################################################################"
	echo -e "#                   AlloGSM  Installation Script                       #"
...
debian_check_dependencies()
{
	missing_packages=" "
	logger "Checking for C development tools ..."
	eval "dpkg-query -l gcc > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "gcc --version > /dev/null 2>&1"
	...
	logger "Checking for C++ developement tools ..."
	eval "dpkg-query -l g++ > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else	
		eval "g++ --version > /dev/null 2>&1"
	...
	logger "Checking for Make utility ..."
	eval "dpkg-query -l make > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "make --version > /dev/null 2>&1"	
	...
	logger "Checking for ncurses library ... "
	eval "dpkg-query -l ncurses > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "type clear > /dev/null 2>&1"
	...
	eval "dpkg-query -l libncurses-dev > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [ ! -f "/usr/include/ncurses.h" ] && [ ! -f "/usr/include/ncurses/ncurses.h" ]; then
	...
	logger "Checking for Perl developement tools ..."
	eval "dpkg-query -l perl > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "perl --version > /dev/null 2>&1"
	...
	logger "Checking for Patch ..."
	eval "dpkg-query -l patch > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "patch --version > /dev/null 2>&1"
	...
	logger "Checking for bison..."
	eval "dpkg-query -l bison > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "type bison > /dev/null 2>&1"
	...
	logger "Checking for bison-devel..."
	eval "dpkg-query -l bison-devel > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [  -f /usr/lib/liby.a ]; then
	...
	logger "Checking for openssl..."
	eval "dpkg-query -l openssl > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		eval "type openssl > /dev/null 2>&1"
	...
	eval "dpkg-query -l libssl-dev > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [ -f /usr/include/openssl/ssl.h ]; then
	...
	eval "dpkg-query -l libgnutls-dev > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [ -f /usr/include/gnutls/gnutls.h ]; then
	...
	eval "dpkg-query -l zlib1g > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [ -f /usr/lib/libz.so.1 ]; then
	...
	eval "dpkg-query -l zlib1g-dev > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		if [ -f /usr/include/zlib.h ]; then
	...
	logger "Checking for kernel development packages..."
	eval "dpkg-query -l linux-headers-$(uname -r) > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		show_status	FAILURE
	...
	eval "dpkg-query -l libxml2-dev > /dev/null 2>&1"
	if [ $? -eq 0 ]; then
		show_status	OK
	else
		show_status	FAILURE
...
patch_dahdi_tool()
{
	...
	if [ $DAHDI_LINUX_COMPLETE_SOURCE_DIR == "" ] || [ $CARD_TYPE == "" ]; then
		exit 1
	fi
...
#	DAHDI_TOOL_CONFIG_FILES="$DAHDI_TOOL_CONFIG_FILES ""tools/xpp/perl_modules/Dahdi/Config/Gen/Chanallogsm.pm"
	...
	if [ $CARD_TYPE == "2aCG" ]; then
		grep -q "^allo2aCG" $file_modules
		if [ $? -ne 0 ]; then
			file_modules_sample_sed="$PROD_PATCH_DIR/dahdi/modules.sample.2aCG.sed"
			sed -i "/^xpp_usb/r $file_modules_sample_sed" $file_modules
		fi
	elif [ $CARD_TYPE == "g4c" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		sed -i "/@genlist = ('/r $file_dahdi_genconf_sed" $file_dahdi_genconf
	fi
	...
	file_span_pm_1_sed="patches/dahdi/Span.pm.1.sed"
	file_span_pm_4_sed="patches/dahdi/Span.pm.4.sed"
	file_span_pm_5_sed="patches/dahdi/Span.pm.4.sed"

        grep -q "my @alloGsm_strings = ("  $file_span_pm 
        if [ $? -ne 0 ]; then
                sed -i "/our \$DAHDI_PRI_CPE = 'pri_cpe'/r $file_span_pm_1_sed" $file_span_pm
        else
        	grep -q "AlloGSM A4G" $file_span_pm 
	        if [ $? -ne 0 ]; then
        	        sed -i "/my @alloGsm_strings =/r $file_span_pm_4_sed" $file_span_pm 
		fi
        	grep -q "AlloGSM 2aCG" $file_span_pm 
	        if [ $? -ne 0 ]; then
        	        sed -i "/my @alloGsm_strings =/r $file_span_pm_5_sed" $file_span_pm 
		fi
        fi

	
	grep -q "foreach my \$cardtype (@alloGsm_strings)" $file_span_pm
	if [ $? -ne 0 ]; then
	...
	grep -q "if(\$chan_fqn =~ m(allo2aCG" $file_span_pm
	if [ $? -ne 0 ]; then
	...
	grep -q "\$span->is_allogsm" $file_system_pm
	if [ $? -ne 0 ]; then
		sed -i "s/\$span->is_bri()/\$span->is_bri() || \$span->is_allogsm()/g" $file_system_pm
		sed -i "s/(\$span->is_bri/(\$span->is_bri || \$span->is_allogsm/g" $file_system_pm
	fi
	...
	grep -q "if(\$span->is_allogsm)" $file_chandahdi_pm
	if [ $? -ne 0 ]; then
	...
	\cp "$PROD_PATCH_DIR/dahdi/Chanallogsm.pm" "$DAHDI_LINUX_COMPLETE_SOURCE_DIR/tools/xpp/perl_modules/Dahdi/Config/Gen/Chanallogsm.pm"
	...
	if [ $CARD_TYPE == "2aCG" ]; then
		grep -q "1d21:324" $file_pci_pm
		if [ $? -ne 0 ]; then
			file_pci_pm_sed="$PROD_PATCH_DIR/dahdi/PCI.pm.2aCG.sed"
...
{
#--------------------------------------------------------------------------------------------
# Dahdi started using release with 4 decimal separated digits and use of 10 in second  
# pair made old implementation obsolet.
# Keeping future in mind, using double digit numbers in 4 sets. 
# Pawan 
#--------------------------------------------------------------------------------------------
	DAHDI_VER_NUM=`awk -F. '{printf "%d%02d%02d%02d", $1, $2, $3, $4}' $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/.version`
#--------------------------------------------------------------------------------------------
# OLD
# 	DAHDI_VER_STR=`cat $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/.version`
# 
...
install_subdriver()
{
	...
	if [ "$DAHDI_LINUX_COMPLETE_SOURCE_DIR" == "" ] || [ $DVR_TYPE == "" ]; then
	...
	if [ $DVR_TYPE == "2aCG" ]; then
		eval "\cp -rf $ALLO2ACG_DIR $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/"
		eval "\cp -rf $ALLOSPI_DIR $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/"
	elif [ $DVR_TYPE == "g4c" ]; then
		eval "\cp -rf $ALLOG4C_DIR $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/"
	fi
	...
	if [ $DVR_TYPE == "2aCG" ]; then
		ALLO2ACG_H_FILE="$DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/allo2aCG/allo2aCG.h"
		if [ "$VIRTUAL_TTY" == "YES" ]; then
			sed -i "/#define GSM 1/a#define DAHDI_VER_NUM $DAHDI_VER_NUM\n#define VIRTUAL_TTY" $ALLO2ACG_H_FILE
		else
			sed -i "/#define GSM 1/a#define DAHDI_VER_NUM $DAHDI_VER_NUM" $ALLO2ACG_H_FILE
		fi
	elif [ $DVR_TYPE == "g4c" ]; then
		ALLOG4C_H_FILE="$DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/allog4c/allog4c.h"
		if [ "$VIRTUAL_TTY" == "YES" ]; then
			sed -i "/#define _OPVXG4XX_H_/a#define DAHDI_VER_NUM $DAHDI_VER_NUM\n#define VIRTUAL_TTY" $ALLOG4C_H_FILE
		else
			sed -i "/#define _OPVXG4XX_H_/a#define DAHDI_VER_NUM $DAHDI_VER_NUM" $ALLOG4C_H_FILE
		fi
	fi
	...
	if [ $DVR_TYPE == "2aCG" ]; then
		eval "grep -q CONFIG_DAHDI_ALLO2ACG $KBUILD_FILE"
		if [ $? -ne 0 ]; then
			backup_file "$KBUILD_FILE"
			sed -i "/CONFIG_DAHDI_TRANSCODE/a\obj-\$(DAHDI_BUILD_ALL)\$(CONFIG_DAHDI_ALLO2ACG)		+= allo2aCG/" $KBUILD_FILE
			sed -i "/CONFIG_DAHDI_TRANSCODE/a\obj-\$(DAHDI_BUILD_ALL)\$(CONFIG_DAHDI_ALLOSPI)		+= allospi/" $KBUILD_FILE
		fi
	elif [ $DVR_TYPE == "g4c" ]; then
		eval "grep -q CONFIG_DAHDI_ALLOG4C $KBUILD_FILE"
		if [ $? -ne 0 ]; then
			backup_file "$KBUILD_FILE"
			sed -i "/CONFIG_DAHDI_TRANSCODE/a\obj-\$(DAHDI_BUILD_ALL)\$(CONFIG_DAHDI_ALLOG4C)		+= allog4c/" $KBUILD_FILE
		fi
	fi

	echo
	echo "==========================================================================="	
	logger "Installing AlloGSM driver ..."
	show_status OK
	echo "==========================================================================="
...
install_allog4c(){
	install_subdriver "g4c"
}

install_allo2aCG(){
	install_subdriver "2aCG"
}
...
setect_driver()
{
	clearscr
	banner
        select_list "Detect Card and Install Driver Automatically (Card Must be Inserted)" "Install Allo G4C Driver" \
		    "Install Allo 2aCG (2nd Gen) Driver" "Install both G4C and 2aCG Drivers"

	case $SEL in
		1) 
			TEST_ALLO2ACG=`lspci -n |grep "1d21:324"`
			if [ -n "$TEST_ALLO2ACG" ]
			then
				echo "Installing 2aCG Driver."			       
				# patch dahdi-tool
				patch_dahdi_tool "2aCG"
				# install 2aCG 2nd Gen GSM Driver
				install_allo2aCG
			fi	
			TEST_ALLOG4C=`lspci -n |grep "10b5:d44f"`
			if [ -n "$TEST_ALLOG4C" ]
			then
				echo "Installing G4C Driver."			       
				# patch dahdi-tool
				patch_dahdi_tool "g4c"
				# install G400P allog4c driver
				install_allog4c
			fi	
			;;
		2)
...
install_liballogsmat()
{
	echo

	cd $LIBALLOGSMAT_DIR

	LIBALLOGSMAT_H_FILE_PATH="$LIBALLOGSMAT_DIR/liballogsmat.h"

	grep '#define VIRTUAL_TTY' $LIBALLOGSMAT_H_FILE_PATH > /dev/null 2>&1
	if [ $? -ne 0 ];  then
		if [ "$VIRTUAL_TTY" == "YES" ]; then
			sed -i "/#define _LIBALLOGSMAT_H/a#define VIRTUAL_TTY" $LIBALLOGSMAT_H_FILE_PATH
		fi
	else
		if [ "$VIRTUAL_TTY" == "NO" ]; then
			sed -i "/#define VIRTUAL_TTY/d" $LIBALLOGSMAT_H_FILE_PATH
		fi
	fi
		...
		if [ $? -ne 0 ]; then
			echo
			error_compile "Installing AlloGSM liballogsmat library: make clean ..."
		fi

		make 2>> "$INSTALL_LOG"
		if [ $? -ne 0 ]; then
			echo
			error_compile "Installing AlloGSM liballogsmat library: make ..."
		fi

		make install 2>> "$INSTALL_LOG"
		if [ $? -ne 0 ]; then
			echo
			error_compile "Installing AlloGSM liballogsmat library: make install ..."
		fi

		##################################################################################
		#Freedom  add 2011-11-04 13:47 If 64bit OS,link liballogsmat.so to /usr/lib64
		##################################################################################
		if [ -d "/usr/lib64" ]; then 
			if [ ` getconf LONG_BIT` = "64" ];then
				if [ -f "/usr/lib/liballogsmat.so" ]; then
					\cp -rf /usr/lib/liballogsmat.* /usr/lib64/
				fi
			fi
		fi
		##################################################################################
		...
		##################################################################################
		mkdir -p /etc/allo/allog4c/modules
		mkdir -p /etc/allo/allog4c/response
		\cp -r $PROD_HOME/src/liballogsmat/config_files/modules/*.conf /etc/allo/allog4c/modules
		\cp -r $PROD_HOME/src/liballogsmat/config_files/response/*.conf /etc/allo/allog4c/response
		##################################################################################
	fi
	
	echo
	echo "==========================================================================="
	logger "Installing AlloGSM liballogsmat library ..."
	show_status OK
	echo "==========================================================================="
...
{		
	...
	eval "grep -q ALLOGSMAT_LIB $AST_SOURCE_DIR/configure"
	if [ $? -eq 0 ]; then
...
#--------------------------------------------------------------------------------------------
# Pawan change configure.3.sed for asterisk 1.6.2... here $as_echo and $as_echo_n not present 
# so we have to modify those variables. Currently modified in configure.3.sed
#--------------------------------------------------------------------------------------------
        ASTVER=`awk -F. '{printf "%01d%02d%02d", $1, $2, $3}' $AST_SOURCE_DIR/.version`
	if [[ $ASTVER -ge "10602" ]] && [[ $ASTVER -le "10807" ]]; then
                file_configure_3_sed="$PROD_PATCH_DIR/asterisk/configure.3.AST_1_6_2.sed"
        else
...
install_chan_allogsm()
{
	echo

	if [ ! -d $CHAN_AGSM_DIR ]; then
		error "$CHAN_AGSM_DIR not found\n"
	fi
	...
	###############################################
	touch $CHAN_AGSM_DIR/src/$CHAN_AGSM_FILE
	###############################################
	\cp -a $CHAN_AGSM_DIR/src/$CHAN_AGSM_FILE $AST_SOURCE_DIR/channels/
	\cp -a $CHAN_AGSM_DIR/config/chan_allogsm.conf $AST_SOURCE_DIR/configs/chan_allogsm.conf.sample
	\cp -a $CHAN_AGSM_DIR/config/allogsm-channels.conf $AST_SOURCE_DIR/configs/allogsm-channels.conf.sample
	
	echo
	echo "==========================================================================="
	logger "Installing AlloGSM chan_allogsm module to $AST_SOURCE_DIR ..."
	show_status OK
	echo "==========================================================================="
...
##################################################################################
	#Pawan: For AST VER 11 and up (To add version.h)
##################################################################################
#	if test ! -f include/asterisk/version.h ; then
	if grep -c error include/asterisk/version.h ; then
	ASTVERNUM=`awk -F. '{printf "%01d%02d%02d", $1, $2, $3}' .version`
	echo ASTVERNUM: $ASTVERNUM
cat > include/asterisk/version.h << END
/*
 * version.h 
 * Automatically generated
 */
#define ASTERISK_VERSION_NUM ${ASTVERNUM}
...
##################################################################################
		...
		##################################################################################
		#Freedom  add 2011-11-04 13:47 If 64bit OS,link chan_allogsm.so to /usr/lib64/asterisk/modules/chan_allogsm.so
		##################################################################################
		...
		if [ ! -f "/etc/asterisk/chan_allogsm.conf" ]; then
			\cp -a $CHAN_AGSM_DIR/config/chan_allogsm.conf /etc/asterisk
		fi
		if [ ! -f "/etc/asterisk/allogsm-channels.conf" ]; then
			\cp -a $CHAN_AGSM_DIR/config/allogsm-channels.conf /etc/asterisk
		fi
...
#	./source-code_dahdi.sh
	...
	if [ x"$SKIP_DAHDI" != x"true" ]; then
		...
		if [ "$DHAID_AUTO_INSTALL" == "YES" ]; then
			if [ "$G400P_AUTO_INSTALL" == "YES" ]; then
				...
				LIBALLOGSMAT_AUTO_INSTALL="YES"
				CHAN_AGSM_AUTO_INSTALL="YES"
			else
				LIBALLOGSMAT_AUTO_INSTALL="NO"
				CHAN_AGSM_AUTO_INSTALL="NO"
			fi
			install_dahdi
		fi
	fi

	# install liballogsmat for allog4c	
	if [ "$LIBALLOGSMAT_AUTO_INSTALL" == "YES" ]; then
		install_liballogsmat
	fi
	...
	if [ "$AST_AUTO_INSTALL" == "YES" ]; then
		if [ "$CHAN_AGSM_AUTO_INSTALL" == "YES" ]; then
			install_chan_allogsm
		fi
		install_asterisk
	fi
...
{		
	...
	echo
	echo "==========================================================================="
	logger "Installing AlloGSM driver for $ENVIRON_TRIXBOX_VERSION ..."
	show_status OK
	echo "==========================================================================="
	...
	if [ ! $? -eq 0 ]; then
		...
		if [ ! $? -eq 0 ]; then
			...
			if [ $? -eq 0 ]; then
				...
				echo "Please goto http://repo.elastix.org/elastix find your required package."
				exit 1
			fi
		fi
	fi
...
###################################################################
#Added for Elastix: append chan_allogsm before installing asterisk
# pawan
        tmp=`grep -c "\#define ELASTRIX 1" $CHAN_AGSM_DIR/src/$CHAN_AGSM_FILE`
        if [ "$tmp" -lt "1" ]
        then
                sed -i 's/.*#if ELASTRIX.*/#define ELASTRIX 1\n&/' $CHAN_AGSM_DIR/src/$CHAN_AGSM_FILE
        fi
...
###################################################################
	...
	echo
	echo "==========================================================================="
	logger "Installing AlloGSM driver for $ENVIRON_ELASTIX_VERSION ..."
	show_status OK
	echo "==========================================================================="
}
...
usage()
{
        ...
        echo "Install chan_allogsm.c to asterisk and allog4c to dahdi"
        echo 
        echo "OPTION:"
...
LIBALLOGSMAT_DIR=$DRIVER_SRC_DIR/liballogsmat/
ALLOG4C_DIR=$DRIVER_SRC_DIR/allog4c/
ALLO2ACG_DIR=$DRIVER_SRC_DIR/allo2aCG/
ALLOSPI_DIR=$DRIVER_SRC_DIR/allospi/
OPVXd115_DIR=$DRIVER_SRC_DIR/opvxd115/
OPVXA24XX_DIR=$DRIVER_SRC_DIR/opvxa24xx/
LOG_DIR=$PROD_HOME/log
CHAN_AGSM_DIR=$DRIVER_SRC_DIR/chan_allogsm/
CHAN_AGSM_FILE="chan_allogsm.c"
