...
#define AT_CALL_STATE_CALL_WAITING			13
...
#define UPDATE_OURCALLSTATE(gsm,c,newstate) do {\
	if (gsm->debug & (ALLOGSM_DEBUG_AT_STATE) && c->ourcallstate != newstate) \
...
extern int gsm_schedule_check(struct allogsm_modul *gsm);
 
extern int gsm_schedule_event(struct allogsm_modul *gsm, int ms, void (*function)(void *data), void *data);

extern allogsm_event *allogsm_schedule_run(struct allogsm_modul *gsm);

extern void gsm_schedule_del(struct allogsm_modul *gsm, int ev);
...
//extern struct allogsm_modul *__gsm_new_tei(int fd, int node, int switchtype, int span, gsm_io_cb rd, gsm_io_cb wr, void *userdata);
extern struct allogsm_modul *__gsm_new_tei(int fd, int node, int switchtype, int span, allogsm_rio_cb rd, allogsm_wio_cb wr, void *userdata, int at_debug, int call_waiting_enabled, int auto_modem_reset);

extern void __gsm_free_tei(struct allogsm_modul *p);

extern void gsm_dump(struct allogsm_modul *gsm, const char *h, int len, int txrx);

//Freedom Add 2010-10-10 16:03
extern int gsm_send_at(struct allogsm_modul *gsm, const char *at);
extern int gsm_transmit(struct allogsm_modul *gsm, const char *at);

extern int gsm_transmit_data(struct allogsm_modul *gsm, const char *data, int len);

#if 0
extern struct alloat_call *allogsm_getcall(struct allogsm_modul *gsm, int cr, int outboundnew);
#endif

extern int gsm_compare(const char *str_at, const char *str_cmp);

extern int gsm_trim(const char *in, char *out, int len);

extern void gsm_get_manufacturer(struct allogsm_modul *gsm, char *h);

extern void gsm_get_smsc(struct allogsm_modul *gsm, char *h);

extern void gsm_get_model_name(struct allogsm_modul *gsm, char *h);

extern void gsm_get_model_version(struct allogsm_modul *gsm, char *h);

extern void gsm_get_imsi(struct allogsm_modul *gsm, char *h); 

extern void gsm_get_imei(struct allogsm_modul *gsm,char *h);

extern void gsm_get_operator(struct allogsm_modul *gsm, char *buf);

extern allogsm_event *gsm_mkerror(struct allogsm_modul *gsm, char *errstr);

extern void gsm_message(struct allogsm_modul *gsm, char *fmt, ...);

extern void gsm_error(struct allogsm_modul *gsm, char *fmt, ...);

extern void sms_send_ok(struct allogsm_modul *gsm);

extern int gsm_switch_state(struct allogsm_modul *gsm, int state, const char *next_command);
	
extern int gsm_switch_sim_state(struct allogsm_modul *gsm, int state, char *next_command);

extern unsigned long gsm_hex2int(const char *hex, int len);

extern int gsm_pdu2sm_event(struct allogsm_modul *gsm, char *pdu);

extern int gsm_ussd_event(struct allogsm_modul *gsm, char *ussd) ;


extern int sms_get_str(struct allogsm_modul *gsm,char *in, size_t inlen, char *out, size_t outlen);

extern int sms_set_str(char *in, char *out);

extern int gsm_text2sm_event(struct allogsm_modul *gsm, char *sms_info);
extern int gsm_text2sm_event2(struct allogsm_modul *gsm, char *sms_info, char *sms_body);

extern int gsm_sms_send_text(struct allogsm_modul *gsm, char *destination, char *msg);

extern int gsm_sms_send_pdu(struct allogsm_modul *gsm, char *pdu);

extern enum sms_mode gsm_check_sms_mode(struct allogsm_modul *gsm, char *sms_info);

extern int gsm_san(struct allogsm_modul *gsm, char *in, char *out, int len);
...
extern allogsm_event * module_check_wind(struct allogsm_modul *gsm, struct alloat_call *call, char *buf, int i);
extern void gsm_start_timeout_junk(void *data);
